...
class BilibiliClient extends EventEmitter {
  ...
  async sendMsg (msg, cookie) {
    let result = await request.post(`https://api.live.bilibili.com/msg/send`).send(`color=16777215&fontsize=25&mode=1&msg=${msg}&rnd=${Math.floor(new Date().getTime() / 1000)}&roomid=${this.roomID}`)
    .timeout(5000)
    .set('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36')
    .set('Referer', `${baseURL}`)
    .set('Cookie', cookie)
    logger.info(`服务器返回 ${result.body.msg}`)
    // logger.info(result)
  }
}
