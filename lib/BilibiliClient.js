...
const logger = require('./logger.js')('BilibiliClient')
...
class BilibiliClient extends EventEmitter {
  ...
  async getRoomID (fakeRoomID) {
    let result = await request.get(`${baseURL}/${fakeRoomID}`)
      .timeout(5000)
    ...
    let trueRoomID = result.text.match(/var ROOMID = (\d*?);/)[1]
    let roomTitle = result.text.match(/itemprop="name" content="(.*?) - (.*?) - /)[1]
    let roomUP = result.text.match(/itemprop="name" content="(.*?) - (.*?) - /)[2]
  ...
  async getLiveAddress (roomID) {
    let result = await request.get(`http://live.bilibili.com/api/playurl?cid=${roomID}&player=1&quality=0`)
    .timeout(5000)
    .set('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36')
    .set('Referer', `${baseURL}`)

    let match = result.text.match(/<url><!\[CDATA\[.*?\]\]><\/url>/)
